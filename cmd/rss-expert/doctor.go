...
func checkAll(ctx context.Context, cfg config.Config) []check {
	...
	if !state.UpToDate() {
		return append(checks, check{
			name: "migrations",
			err:  fmt.Errorf("schema is at %d, latest is %d; run: rss-expert migrate", state.Applied, state.Latest),
		})
	}
	return append(checks, check{
		name:   "migrations",
...
func describeEnvFiles() string {
	if named := os.Getenv("RSS_EXPERT_ENV_FILE"); named != "" {
		return named + " (named by RSS_EXPERT_ENV_FILE)"
	}
	...
	switch len(found) {
	case 0:
		return "none here; the real environment is all there is"
	case 1:
		return found[0]
	}
	return strings.Join(found, " first, then ")
}

func describeLimits(cfg config.Config) string {
	proxy := "forwarded headers ignored"
	if cfg.BehindProxy {
		proxy = "forwarded headers trusted"
	}
	preview := ""
	if cfg.ShowPreview {
